...
if(status==="present"){
  ...
  if(presentStudents.includes(roll_no)){
     targetLocation=presentStudents.indexOf(roll_no)
  ...
  submitForm(){
    var totalSubmited=this.state.presentStudents.length+this.state.absentStudents.length
    var expectedsubmitions=this.state.students.length
    if(totalSubmited===expectedsubmitions){
      this.props.navigation.navigate("SummaryScreen",{
        absentStudents:this.state.absentStudents.length,
        presentStudents:this.state.presentStudents.length,
        totalpercent:this.state.presentStudents.length/expectedsubmitions
      })
    }
    else{
      this.setState({
    ...
    absentStudents:absentStudents,
    submitable:false,
    missingData:expectedsubmitions-totalSubmited 
      })
    }
  }
  render() {
